  ...
  <div class="f-radios-wrap">
    ...
    >
      ...
      <li
        ...
        role="option"
      >
        <span class="f-key" v-if="!editingOther">{{
          getToggleKey(question.options.length)
        }}</span>
        <input
          v-if="editingOther"
...
export default {
  ...
  methods: {
        ...
        if (keyCode >= 65 && keyCode <= 90) {
          ...
          if (index > -1) {
            ...
            if (option) {
              ...
              index === this.question.options.length
            ) {
              this.startEditOther();
            }
          }
    ...
    getLabel(index) {
      return this.language.ariaMultipleChoice.replace(
        ":letter",
        this.getToggleKey(index)
      );
    },
