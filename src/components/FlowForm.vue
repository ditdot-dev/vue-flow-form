...
<div class="vff" :class="{ 'vff-not-standalone': !standalone }">
  <div class="f-container">
    <div class="f-form-wrap">
      <flow-form-question ref="questions" v-for="(q, index) in questionList" v-bind:question="q" v-bind:language="language" v-bind:key="'q' + index" v-bind:active="q.index === activeQuestionIndex" v-model="q.answer" v-on:answer="onQuestionAnswered"
        v-bind:reverse="reverse" :noButton="
  ...
  <div class="d-flex flex-column progress-cicles">
    <div :class="`d-flex justify-content-center progress-circle ${
          activeQuestionIndex === index && 'active'
        }`" v-for="(question, index) in questions" :key="index" @click="handleProgressBar(index)">
      {{ index + 1 }}
    </div>
  </div>
...
export default {
  ...
  methods: {
    ...
    onKeyUpListener(e) {
      if (
        e.shiftKey || ["Tab", "Enter"].indexOf(e.key) === -1 ||
        this.submitted
      ) {
        return;
      }
